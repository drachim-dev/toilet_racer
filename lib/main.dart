...
import 'package:toilet_racer/views/level_menu.dart';
import 'package:toilet_racer/views/overlay_ui.dart';
import 'package:toilet_racer/views/stage_menu.dart';
          ...
          child: GameWidget(
            ...
            overlayBuilderMap: {
              kStartMenu: (_, RaceGame game) => StartMenu(
                    ...
                    onSelectLevelsPressed: game.showStagesMenu,
                    onCreditsPressed: game.showCreditsMenu,
                    onLeaderboardPressed: game.showLeaderboardMenu,
                  ),
              kStagesMenu: (_, RaceGame game) {
                return StageMenu(
                  onStageSelected: (Stage stage) {
                    _selectedStage = stage;
                    game.showLevelsMenu();
                  },
                );
              },
              kLevelsMenu: (_, RaceGame game) {
                final StageRepository _stageRepository =
                    locator<StageRepository>();
                final stage =
                    _selectedStage ?? _stageRepository.getAllStages().first;
                _selectedStage = null;
                return LevelMenu(
                    onPlayPressed: (Level level) => game.prepareStartGame(
                        gameModeIdentifier: GameModeIdentifier.career,
                        playOption: PlayOption.next(level: level)),
                    stage: stage);
              },
